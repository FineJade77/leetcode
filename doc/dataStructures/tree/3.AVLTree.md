...
```go
func (n *TreeNode) Search(d int) (*TreeNode, bool) {
	if n == nil {
		return nil, false
	}
...
func (n *TreeNode) Delete(d int, parent *TreeNode) bool {
    ...
    switch {
        case d < n.Data:
            return n.Left.Delete(d, n)

        case d > n.Data:
            return n.Right.Delete(d, n)
...
### 代码
[https://play.golang.org/p/wioMGfC08M](https://play.golang.org/p/wioMGfC08M)

### 优缺点
优点:<br>
> 旋转保持树的平衡，查找最坏的复杂度为O(logN)，并且中序遍历的顺序不变

缺点:<br>
> 破坏了树的结构(hierarchy-父子关系)
